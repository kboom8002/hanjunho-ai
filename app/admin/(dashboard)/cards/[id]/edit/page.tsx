...
export default async function EditCardPage({ params }: { params: { id: string } }) {
  ...
  const { data: card, error } = await supabase.from('answer_cards').select('*').eq('id', params.id).single();
    ...
    <div className="max-w-4xl">
      <div className="mb-8">
        <div className="flex gap-2 text-caption text-neutral-500 mb-2">
          <Link href="/admin/cards" className="hover:text-brand-700">Answer Cards</Link> &gt; <span>카드 수정</span>
        </div>
        <h1 className="text-h2">Answer Card 수정</h1>
        <p className="text-body text-neutral-500 mt-2">기존 정책 카드의 내용을 수정합니다.</p>
      </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Select name="public_status" defaultValue={card.public_status} required>
            ...
            <div>
              ...
              <Input name="canonical_slug" defaultValue={card.canonical_slug} placeholder="e.g. gtx-r-plan" required />
            </div>
            <div className="md:col-span-2">
              <label className="block text-body-sm font-semibold mb-2">카드 제목 (내부 관리용 및 H1) <span className="text-red-500">*</span></label>
              <Input name="name" defaultValue={card.name} placeholder="GTX-R 노선 신설 계획" required />
            </div>
            <div>
              <label className="block text-body-sm font-semibold mb-2">클리어맵 카테고리 <span className="text-red-500">*</span></label>
              <Select name="category" defaultValue={card.category} required>
            ...
            <div>
              ...
              <Input name="policy_domain" defaultValue={card.policy_domain} placeholder="예: 경기 교통" required />
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-6">
            <div>
              ...
              <Textarea name="user_query" defaultValue={card.user_query} placeholder="GTX-R은 기존 교통대책과 무엇이 다른가요?" required className="min-h-[80px]" />
            </div>
            <div>
              ...
              <Textarea name="snippet" defaultValue={card.snippet} placeholder="직접 연결하여 30분을 단축합니다. 예타 면제로 조기 착공합니다. ..." required />
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-8">
            <div className="p-4 bg-surface-soft rounded-xl border border-line-default">
              ...
              <Input name="action_plan_1_title" defaultValue={card.action_plan_1_title || ''} placeholder="방안 1 제목" className="mb-3" />
              <Textarea name="action_plan_1_body" defaultValue={card.action_plan_1_body || ''} placeholder="방안 1 상세 설명" className="min-h-[80px]" />
            </div>
            <div className="p-4 bg-surface-soft rounded-xl border border-line-default">
              <label className="block text-body-sm font-semibold mb-2">실행 방안 2 제목 및 내용</label>
              <Input name="action_plan_2_title" defaultValue={card.action_plan_2_title || ''} placeholder="방안 2 제목" className="mb-3" />
              <Textarea name="action_plan_2_body" defaultValue={card.action_plan_2_body || ''} placeholder="방안 2 상세 설명" className="min-h-[80px]" />
            </div>
            <div className="p-4 bg-surface-soft rounded-xl border border-line-default">
              <label className="block text-body-sm font-semibold mb-2">실행 방안 3 제목 및 내용</label>
              <Input name="action_plan_3_title" defaultValue={card.action_plan_3_title || ''} placeholder="방안 3 제목" className="mb-3" />
              <Textarea name="action_plan_3_body" defaultValue={card.action_plan_3_body || ''} placeholder="방안 3 상세 설명" className="min-h-[80px]" />
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-6">
            <div>
              ...
              <Textarea name="context_impact" defaultValue={card.context_impact} placeholder="이 정책이 시행되면 도민의 일상은 이렇게 바뀝니다." required />
            </div>
          </div>
        </section>
